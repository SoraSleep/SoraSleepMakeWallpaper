...
export async function exportWallpaperPackage(
  ...
  const config = {
    ...
    parallax: project.preset.id === 'layered-parallax' ? {
      ...project.parallax,
      layers: project.parallax.layers.map((layer, index) => ({ ...layer, asset: index === 0 ? `assets/image-a.${assetExtension(project.assets.imageA)}` : 'assets/image-b-aligned.png' })),
    } : undefined,
  ...
  const parallaxRuntime = `const C=document.querySelector('canvas'),X=C.getContext('2d'),Q=window.WALLPAPER_CONFIG;let T={x:0,y:0},P={x:0,y:0},last=performance.now(),A=[];function load(){A=Q.parallax.layers.filter(l=>l.visible!==false).map(l=>{const i=new Image();i.src=l.asset;return {...l,i}})}function fit(i){const s=Math.max(C.width/i.width,C.height/i.height)*(1+Q.parallax.overscan/100),w=i.width*s,h=i.height*s;return{x:(C.width-w)/2,y:(C.height-h)/2,w,h}}function frame(t){const dt=Math.min((t-last)/1000,.1);last=t;P.x+=(T.x-P.x)*(1-Math.exp(-Math.max(1,Q.parallax.smoothing/12)*dt));P.y+=(T.y-P.y)*(1-Math.exp(-Math.max(1,Q.parallax.smoothing/12)*dt));X.clearRect(0,0,C.width,C.height);for(const l of A)if(l.i.complete){const r=fit(l.i),dx=P.x*C.width*l.depth*Q.parallax.cameraStrength/100,dy=P.y*C.height*l.depth*Q.parallax.cameraStrength/100;X.drawImage(l.i,r.x-dx,r.y+dy,r.w*l.scale,r.h*l.scale)}requestAnimationFrame(frame)}C.onpointermove=e=>{const r=C.getBoundingClientRect();T={x:(e.clientX-r.left)/r.width-.5,y:(e.clientY-r.top)/r.height-.5}};function resize(){const d=Math.min(devicePixelRatio||1,2);C.width=innerWidth*d;C.height=innerHeight*d}addEventListener('resize',resize);resize();load();requestAnimationFrame(frame);`;
  const portalRuntime = `const C=document.querySelector('canvas'),X=C.getContext('2d'),Q=window.WALLPAPER_CONFIG;
const A=new Image(),B=new Image();let P={x:.5,y:.55},T={...P},R=Q.lens.radius,H=0,last=performance.now(),idle=0,paused=document.hidden;
Q.portal=Object.assign({glow:72,ripple:0,rippleEnabled:true,reducedMotion:false},Q.portal||{});A.src=Q.assets.imageA;B.src=Q.assets.imageB;
window.wallpaperPropertyListener={applyUserProperties(p){if(p.lensradius)Q.lens.radius=p.lensradius.value*100;if(p.feather)Q.lens.feather=p.feather.value*100;if(p.followspeed)Q.lens.followSpeed=p.followspeed.value;if(p.revealintensity)Q.lens.revealIntensity=p.revealintensity.value*100;if(p.portalglow)Q.portal.glow=p.portalglow.value*100;if(p.portalripple)Q.portal.ripple=p.portalripple.value*100;if(p.rippleenabled)Q.portal.rippleEnabled=p.rippleenabled.value;if(p.reducedmotion)Q.portal.reducedMotion=p.reducedmotion.value}};
function fit(i){const contain=Q.canvas.fit==='contain',s=(contain?Math.min:Math.max)(C.width/i.width,C.height/i.height),w=i.width*s,h=i.height*s;return{x:(C.width-w)/2,y:(C.height-h)/2,w,h}}
function auto(t){if(t-idle<1200)return;const m=Q.mobileMotion||{path:'guided',duration:8,loop:'repeat'},q=(t/1000%m.duration)/m.duration,a=q*Math.PI*2;if(m.path==='orbit')T={x:.5+Math.cos(a)*.2,y:.5+Math.sin(a)*.24};else if(m.path==='figure8')T={x:.5+Math.sin(a)*.22,y:.5+Math.sin(a*2)*.16};else if(m.path==='breathe')T={x:.5+Math.cos(a)*.035,y:.54+Math.sin(a)*.035};else T={x:.33+q*.34,y:.62-Math.sin(q*Math.PI)*.2}}
function frame(t){requestAnimationFrame(frame);if(paused)return;const dt=Math.min((t-last)/1000,.1);last=t;if(innerHeight>innerWidth&&!Q.portal.reducedMotion)auto(t);const k=1-Math.exp(-(2+Q.lens.followSpeed*.14)*dt);P.x+=(T.x-P.x)*k;P.y+=(T.y-P.y)*k;const target=H&&!Q.portal.reducedMotion?Math.min(42,Q.lens.radius*1.45):Q.lens.radius;R+=(target-R)*(1-Math.exp(-10*dt));if(A.complete&&B.complete){X.clearRect(0,0,C.width,C.height);const a=fit(A),b=fit(B),r=R/100*Math.min(C.width,C.height),cx=P.x*C.width,cy=(1-P.y)*C.height,soft=Math.max(2,r*Q.lens.feather/100);X.drawImage(A,a.x,a.y,a.w,a.h);X.save();const g=X.createRadialGradient(cx,cy,Math.max(0,r-soft),cx,cy,r);g.addColorStop(0,'rgba(255,255,255,1)');g.addColorStop(Math.max(0,1-soft/r),'rgba(255,255,255,1)');g.addColorStop(1,'rgba(255,255,255,0)');X.beginPath();X.arc(cx,cy,r,0,Math.PI*2);X.clip();X.globalAlpha=Q.lens.revealIntensity/100;X.drawImage(B,b.x,b.y,b.w,b.h);X.globalCompositeOperation='destination-in';X.fillStyle=g;X.fillRect(cx-r,cy-r,r*2,r*2);X.restore();const glow=Q.portal.glow/100;X.save();X.shadowColor='rgba(185,228,114,.9)';X.shadowBlur=12+24*glow;X.strokeStyle='rgba(185,228,114,'+Math.min(1,.35+glow*.5)+')';X.lineWidth=1.5+glow*2;X.beginPath();X.arc(cx,cy,r,0,Math.PI*2);X.stroke();const ripple=Q.portal.rippleEnabled&&!Q.portal.reducedMotion?Q.portal.ripple/100:0;if(ripple>0){for(let n=0;n<2;n++){const phase=((t/1800+n*.5)%1),rr=r+phase*r*.5;X.globalAlpha=(1-phase)*ripple*.45;X.lineWidth=1;X.beginPath();X.arc(cx,cy,rr,0,Math.PI*2);X.stroke()}}X.restore()}}
function point(e){const q=C.getBoundingClientRect();T={x:(e.clientX-q.left)/q.width,y:1-(e.clientY-q.top)/q.height};idle=performance.now()}
C.onpointermove=point;C.onpointerdown=e=>{point(e);H=1;C.setPointerCapture(e.pointerId)};C.onpointerup=e=>{H=0;if(C.hasPointerCapture(e.pointerId))C.releasePointerCapture(e.pointerId)};C.onpointercancel=()=>H=0;
function resize(){const d=Math.min(devicePixelRatio||1,2);C.width=Math.max(1,Math.round(innerWidth*d));C.height=Math.max(1,Math.round(innerHeight*d))}addEventListener('resize',resize);addEventListener('visibilitychange',()=>{paused=document.hidden;last=performance.now()});resize();requestAnimationFrame(frame);`;
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${project.title}</title><style>html,body,canvas{margin:0;width:100%;height:100%;overflow:hidden;background:#0b0d10}canvas{display:block;touch-action:none}</style></head><body><canvas></canvas><script>window.WALLPAPER_CONFIG=${JSON.stringify(config)};<\/script><script src="runtime.js"><\/script></body></html>`;
  const projectJson = {
    title: project.title,
    ...
    general: { properties: {
      ...
      ...(project.preset.id === 'portal-reveal' ? {
        ...
        rippleenabled: { order: 7, text: 'Enable ripple', type: 'bool', value: Boolean((project.presetSettings.portal as { rippleEnabled?: boolean } | undefined)?.rippleEnabled ?? true) },
        reducedmotion: { order: 8, text: 'Reduced motion', type: 'bool', value: Boolean((project.presetSettings.portal as { reducedMotion?: boolean } | undefined)?.reducedMotion ?? false) },
      } : {}),
    } },
  };
  const files: Record<string, Uint8Array> = {
    'index.html': strToU8(html),
    'runtime.js': strToU8(project.preset.id === 'portal-reveal' ? portalRuntime : project.preset.id === 'layered-parallax' ? parallaxRuntime : runtime),
    ...
    'INSTALL.txt': strToU8(`MOTION PAIR STUDIO — ${project.preset.id === 'portal-reveal' ? 'PORTAL REVEAL' : 'DIFFERENCE LENS'}\n\nWINDOWS / WALLPAPER ENGINE\n1. Extract this ZIP to a permanent folder.\n2. In Wallpaper Engine, choose Open from File.\n3. Select index.html from the extracted folder.\n4. Use the wallpaper Properties panel to adjust portal size, glow, ripple, and reduced motion.\n\nANDROID\nInteractive Web Wallpaper is intended for Windows Wallpaper Engine. Use the accompanying mobile video bundle for Android or any video-wallpaper app that supports MP4/WebM.\n\nSUPPORT\nKeep the files together. Do not move runtime.js, config.json, or assets/ after installation. This wallpaper runs offline and makes no network requests.\n`),
    'LICENSE.txt': strToU8(`LICENSE TEMPLATE — REPLACE BEFORE SALE\n\nCopyright (c) ${new Date().getFullYear()} [ARTIST / SELLER NAME].\n\nThe buyer receives a personal, non-transferable license to use this wallpaper on their own devices. The buyer may not redistribute, resell, upload, extract, or claim ownership of the artwork, assets, or runtime. Commercial use, inclusion in product bundles, and AI-training use require written permission from the copyright holder.\n\nThe seller must confirm they hold rights to every supplied image, character, logo, and third-party asset before distributing this package.\n`),
    'CHANGELOG.txt': strToU8(`v1.0.0\n- Initial commercial export generated by Motion Pair Studio.\n- Preset: ${project.preset.id}.\n- Offline Web Wallpaper runtime with Wallpaper Engine properties.\n`),
    'THIRD-PARTY-NOTICES.txt': strToU8('This export bundle contains generated HTML, JavaScript, JSON, and artwork assets only. No external runtime libraries or network dependencies are bundled.\n'),
  };
