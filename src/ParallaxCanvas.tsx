...
export function ParallaxCanvas({ settings, fit, fps, onPerformance }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all(visible.map(async (layer) => ({ layer, image: await load(layer.asset!.source) }))).then((loaded) => {
      ...
      const draw = (now: number) => {
        ...
        const response = 1 - Math.exp(-(2 + state.settings.smoothing * 0.14) * dt);
        ...
        for (const { layer, image } of loaded) {
          ...
          const y = (height - drawHeight) / 2 + (0.5 - camera.current.y) * height * movement + height * layer.offsetY / 100;
  ...
  return <><canvas ref={canvasRef} className="difference-lens-canvas" aria-label="Interactive layered parallax preview" onPointerMove={(event) => { const rect = event.currentTarget.getBoundingClientRect(); pointer.current = { x: (event.clientX - rect.left) / rect.width, y: 1 - (event.clientY - rect.top) / rect.height }; }} />{status !== 'ready' && <div className="renderer-state" role="status">{status === 'loading' ? 'Loading parallax layers…' : 'Parallax layer could not be loaded.'}</div>}</>;
}
